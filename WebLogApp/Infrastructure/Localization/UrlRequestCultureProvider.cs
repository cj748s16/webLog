...
namespace WebLogApp.Infrastructure.Localization
{
    public class UrlRequestCultureProvider : IRequestCultureProvider
    {
        public Task<ProviderCultureResult> DetermineProviderCultureResult(HttpContext httpContext)
        {
            ...
            // Quick and dirty parsing of language from url path, which looks like /api/en-US/system/account
            var parts = httpContext.Request.Path.Value.Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries).ToList();
            if (parts.Count == 0)
            {
